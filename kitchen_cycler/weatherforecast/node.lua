...
function node.render()
   -- prepare clock for rendering in top right corner
    ...
    font:fbwrite(100,50, "Forecast",100, 1,0,0,1)
	local i = 1
	while (i <= #forecast) do
		local ypos = 150 + i * 60
		font:fbwrite(100, ypos, string.sub(forecast[i].weekday,1,3) ..":", 50,0,1,0,1)
		font:fbwrite(300, ypos, round(forecast[i].temp,2) .. " C", 50,0,1,0,1)
		font:fbwrite(600, ypos, forecast[i].desc, 50,0,1,0,1)
		i = i + 1
	end
